...
export proc_discrete_svd
export proc_discrete_revised_dt
export proc_discrete_ctot_revised_dt
export ctot_chi_squared_stats 
        ...
        if cntStar > 0
            ...
            for i in star_ind
                if in_bmaskd[cx[i],cy[i]]
                    ...
                    dt_build_cov!(cov,μ,cx[i]+offx,cy[i]+offy,bimage,bism,Np,widx,widy)
...
"""
function proc_discrete(x_locs,y_locs,raw_image,mask_image;Np=33,widx=129,widy=widx,tilex=1,tiley=tilex,seed=2021,ftype::Int=32,rlim=625,ndraw=0, verbose=true)
    radNp = (Np-1)÷2
    if ftype == 32
        T = Float32
    else
...
function proc_discrete_dt(x_locs,y_locs,raw_image,mask_image;Np=33,widx=129,widy=widx,tilex=1,tiley=tilex,seed=2021,ftype::Int=32,rlim=625,ndraw=0, org_mu=false, verbose=true)
    radNp = (Np-1)÷2
    if ftype == 32
        T = Float32
    else
        ...
        if cntStar > 0
            ...
            for i in star_ind
                if org_mu 
                    ...
                    dt_build_cov!(cov,μ,cx[i]+offx,cy[i]+offy,bimage,bism,Np,widx,widy) #changed mu def
                end
                cov_stamp = cx[i]-radNp:cx[i]+radNp,cy[i]-radNp:cy[i]+radNp
...
function proc_discrete_revised_dt(x_locs,y_locs,raw_image,mask_image;Np=33,widx=129,widy=widx,tilex=1,tiley=tilex,seed=2021,ftype::Int=32,rlim=625,ndraw=0, verbose=false, org_mu=false)
    radNp = (Np-1)÷2
    if ftype == 32
        T = Float32
    else
