...
#edits
function proc_discrete_dt(x_locs,y_locs,raw_image,mask_image;Np=33,widx=129,widy=widx,tilex=1,tiley=tilex,seed=2021,ftype::Int=32,rlim=625,ndraw=0)
    radNp = (Np-1)÷2
    if ftype == 32
        T = Float32
    else
...
#function
function proc_discrete_revised_dt(x_locs,y_locs,raw_image,mask_image;Np=33,widx=129,widy=widx,tilex=1,tiley=tilex,seed=2021,ftype::Int=32,rlim=625,ndraw=0)
    radNp = (Np-1)÷2
    if ftype == 32
        T = Float32
    else
    ...
    if ndraw>0
        return predcovar, cov, mod_im[1].-out_mean[1:sx0, 1:sy0], mod_im[1].-out_draw[1:sx0, 1:sy0, :]
    else
...
function chi_squared_stats(x_locs,y_locs,raw_image,mask_image, img;Np=33,r=4^2, widx=129,widy=widx,tilex=1,tiley=tilex,seed=2021,ftype::Int=32,rlim=625,ndraw=0)
    Np = 35;   #35
    halfNp = (Np-1)÷2;
    dv = halfNp;

    cov, predcovar, star_stats = proc_discrete_revised_dt(x_locs,y_locs,raw_image,mask_image,Np=Np, widx=widx,widy=widy,tilex=tilex,tiley=tiley,seed=seed,ftype::Int=32,rlim=rlim,ndraw=ndraw);
    icov = cholesky(cov);
    ipredcov = cholesky(predcov);

    return chisquared_xreal_ctot(img, icov, Np, x_locs[1], y_locs[1], dv), chisquared_xinfill_ctot(star_stats, icov, Np, x_locs[1], y_locs[1], dv),  chisquared_xinfill_cinfill(star_stats, mask_image, ipredcov), chisquared_xreal_cinfill(img, mask_image, ipredcov)
